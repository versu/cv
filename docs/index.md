...
- [スキル](#スキル)
  ...
  - [その他](#その他)
- [職務経歴詳細](#職務経歴詳細)
  - [ハンドメイド作品のオンラインマーケットプレイスを中心にサービス展開するIT企業](#ハンドメイド作品のオンラインマーケットプレイスを中心にサービス展開するit企業)
    - [個人クリエイター向けネットショップ開設サービスの開発](#個人クリエイター向けネットショップ開設サービスの開発)
...
ASP.NET（MVC、Web API、Web Forms）・React・Vue.js・Ruby on Rails・Laravel・CakePHP・jQuery
...
## ハンドメイド作品のオンラインマーケットプレイスを中心にサービス展開するIT企業
...
ECS・RDS(MySQL)・S3・CloudFront

【ツール】

Visual Studio Code・GitHub・Docker・Jira・Slack・Figma

【プロダクト概要】

個人クリエイター（ハンドメイド作家・アーティスト等）向けのネットショップ開設サービス。
専門知識を持たないクリエイターでも、商品登録・在庫管理・決済・受注管理といったEC運営に必要な機能を一括で利用でき、自身のオンラインショップを短時間で立ち上げ・運営できる。
本サービスの提供により、クリエイターは販路開拓や店舗構築にかかる技術的・時間的コストを削減し、作品制作そのものに注力できる環境を実現。


【担当業務概要】

フロントエンドエンジニアとして参画し、React・TypeScript によるユーザー向け機能の設計・実装を中心に担当。
加えて Laravel・PHP によるバックエンド実装、DB設計、GraphQLスキーマ設計など、フロント／バックを横断した開発を担当。
個人クリエイターがノーコードで自由度の高い商品ページを構築できるユーザー体験の実現と、継続的な機能拡張に貢献。

【担当業務詳細】

- **React + TypeScriptを使用したフロントエンド開発**
  - 各種デザインパーツ（画像・テキスト・動画等の汎用パーツ）の設計・実装
    - ユーザーがパーツを自由に組み合わせて自身の商品ページを構築できる汎用UI基盤として提供
  - フリーページ機能の設計・実装
    - パーツを組み合わせた商品ページを任意の数作成できる機能を提供し、クリエイターごとの表現の幅を拡大
  - 抽選販売機能の設計・実装
    - 商品を抽選形式で販売できる機能を提供し、販売形態の多様化を支援
- **Laravel + PHPを使用したバックエンド開発**
  - 上記フロント機能に対応するAPI・ビジネスロジックの設計・実装
- **GraphQLスキーマの設計**
- **DB設計**
- **PHPUnitを使用した単体テスト・結合テストの実装**
  - テストカバレッジ向上と継続的な品質担保のためのテストコード作成
